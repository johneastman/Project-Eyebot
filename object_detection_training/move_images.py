...
For object detection, every image has an associated xml file stated where objects
in the image are. The purpose of this file is to split up that data into two 
data sets: training data and testing data.

Every image (jpeg) and associated xml file share the same unique name. Because
of this, a list of unique names can be put in a list, and those filenames can
be shuffled and distributed into training and testing datasets. After this has
been completed, these filenames can be reassociated with a jpeg and xml file,
which are both then moved into a training or testing directory.
"""
import os
import sys
import random
...
testing_path = sys.argv[2]  # os.path.join(os.getcwd(), f"{enemy_name}_testing")
training_path = sys.argv[3]  # os.path.join(os.getcwd(), f"{enemy_name}_training")

# images_path = os.path.join(os.getcwd(), "gecko_images")
